 ...
 * the author before they start parsing. Derived empirically from test exams at
 * ~4.5 s/page.
 */
 ...
 */
export function estimateParseSeconds(pages: number): number {
  const raw = pages * SECONDS_PER_PAGE;
 ...
 */
export function formatParseEstimate(pages: number): string {
  const seconds = estimateParseSeconds(pages);
...
export function parseCountdown(
  ...
  const total = estimateParseSeconds(pages);
