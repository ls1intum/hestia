...
The app is **English-only** — there is no i18n layer. UI strings are hardcoded directly in components. Shared enum labels live in `src/lib/exam/labels.ts` (task types, Bloom/SOLO levels) and the guided-feedback questionnaire in `src/lib/feedback-questions.ts`; reuse these instead of re-inlining.
...
- **"Strings"**, **"Copy"**, **"Labels"** → strings are inlined in components (English-only, no i18n); shared enum labels in `src/lib/exam/labels.ts`, feedback questions in `src/lib/feedback-questions.ts`
