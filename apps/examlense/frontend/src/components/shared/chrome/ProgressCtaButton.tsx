...
interface Props {
  ...
  fillPct: number;
  disabled?: boolean;
  /** Shows the pulsing success halo. */
  showPing?: boolean;
  onClick: () => void;
}
    ...
    <button
      ...
      className={cn(
        ...
        disabled
          ...
          : tone === "success"
            ? "border-hestia-success/60 bg-hestia-success/15 text-hestia-text hover:bg-hestia-success/25"
            : tone === "warning"
              ? "border-hestia-warning/60 bg-hestia-warning/10 text-hestia-text hover:bg-hestia-warning/20"
