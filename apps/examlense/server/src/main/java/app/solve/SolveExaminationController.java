...
public class SolveExaminationController {

    public record SolveExaminationRequest(@NotBlank String exam_id) {}

    private final SolveExaminationService service;

    public SolveExaminationController(SolveExaminationService service) { this.service = service; }
    ...
    public Map<String, Object> solve(@Valid @RequestBody SolveExaminationRequest req, @CurrentUser String userId) {
        SolveExaminationService.DispatchPlan plan = service.startEvaluation(req.exam_id(), userId);
        return Map.of(
            "ok", true,
