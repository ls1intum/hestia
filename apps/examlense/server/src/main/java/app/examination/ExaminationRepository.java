...
public interface ExaminationRepository extends JpaRepository<Examination, UUID> {
    ...
    List<Examination> findByOwnerIdOrderByCreatedAtDesc(UUID ownerId);

    Optional<Examination> findByIdAndOwnerId(UUID id, UUID ownerId);
     ...
     */
    @Query("select e.ownerId from Examination e where e.id = :id")
    ...
    @Modifying
    @Transactional
    @Query("update Examination e set e.parsePhase = :phase where e.id = :id")
    int updateParsePhase(@Param("id") UUID id, @Param("phase") String phase);

    @Modifying
    @Transactional
    @Query("update Examination e set e.parserModel = :model where e.id = :id")
    int updateParserModel(@Param("id") UUID id, @Param("model") String model);

    @Modifying
    @Transactional
    @Query("update Examination e set e.status = 'failed', e.parseError = :error, e.parsePhase = null where e.id = :id")
    int markParseFailed(@Param("id") UUID id, @Param("error") String error);

    @Modifying
    @Transactional
    @Query("update Examination e set e.status = :status where e.id = :id")
    ...
    @Modifying
    @Transactional
    @Query("update Examination e set e.status = :to where e.id = :id and e.status = :from")
    ...
    @Modifying
    @Transactional
    @Query("update Examination e set e.status = 'failed', e.parseError = :error, e.parsePhase = null "
    ...
    @Modifying
    @Transactional
    @Query("update Examination e set e.status = 'ready', e.parseError = null, e.parsePhase = null "
    ...
    @Modifying
    @Transactional
    @Query("update Examination e set e.status = 'evaluating', e.parseError = null, e.solverModel = :solverModel "
    ...
    @Modifying
    @Transactional
    @Query("update Examination e set e.status = 'failed', e.parseError = :error "
        + "where e.id = :id and e.status = 'evaluating'")
