 ...
 * {@code src/lib/grading.ts} (see {@link #isTaskBlockGraded}) so the table's grading
...
public class ExaminationProgressService {
    ...
    /**
     * Counts backing one exam's progress bar. TaskBlock-relative counts (scored/
    ...
    public ExaminationProgressService(TaskBlockRepository tasks, GradeRepository grades,
                               AIAnswerRepository answers, SectionRepository sections) {
        this.tasks = tasks;
        this.grades = grades;
    ...
    public Map<UUID, Counts> countsFor(List<UUID> examIds) {
        ...
        for (TaskBlock t : tasks.findByExamIdIn(examIds)) {
            ...
            if (hasAnswer) c[2]++;
            if (isTaskBlockGraded(t, gradeByTaskBlock.get(t.getId()), hasAnswer)) c[3]++;
        }
     ...
     * {@code autoGradeChoiceTaskBlock} in {@code src/lib/grading.ts}): not a text
     * task, positive points, at least one correct option defined, and an AI
     * answer to grade against.
     */
    static boolean isTaskBlockGraded(TaskBlock task, Grade grade, boolean hasAnswer) {
        ...
        List<AnswerOption> options = task.getOptions();
