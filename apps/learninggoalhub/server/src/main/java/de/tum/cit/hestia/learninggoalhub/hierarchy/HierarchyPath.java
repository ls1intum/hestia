 ...
 */
public record HierarchyPath(String module, String session, String exercise) {
    ...
    public static HierarchyPath from(HierarchyNode node) {
        ...
        for (HierarchyNode cur = node; cur != null; cur = cur.getParent()) {
            switch (cur.getLevel()) {
                ...
                case SESSION -> session = cur.getLabel();
                case EXERCISE -> exercise = cur.getLabel();
            }
        }
        return new HierarchyPath(module, session, exercise);
    }
}
